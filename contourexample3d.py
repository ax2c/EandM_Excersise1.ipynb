...
# Create a 3D surface plot. The process is a little different
# here since we have to set up our 3D coordinate system.
# Open new figure
fig = plt.figure()
# Set the graph projection to be 3D.
ax = fig.add_subplot(111, projection='3d') # Plot the surface.
# Use that projection to plot the data using the same colormap
# as we used above.
surf = ax.plot_surface(X, Y, Z, cmap='viridis')
# Set the range and label for the z data.
ax.set_zlim(-40,40)
ax.set_zlabel('z')
# Add the colorbar like above.
plt.colorbar(surf, shrink=0.5, aspect=5)
plt.xlabel('x (cm)')
plt.ylabel('y (cm)')
plt.title('My fancy 3D graph')
plt.show()
